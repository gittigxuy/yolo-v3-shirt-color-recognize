...
class YOLO(object):
    def __init__(self):
        ...
        self.model_path = '/home/xuy/code/keras-yolo3-shirt_color/logs/trained_weights_final.h5'  # 模型文件
        # self.classes_path = 'configs/coco_classes.txt'  # 类别文件
        self.classes_path = 'configs/shirt.txt'  # 类别文件
    ...
    def generate(self):
        ...
        boxes, scores, classes = yolo_eval(self.yolo_model.output, self.anchors, len(self.class_names),
                                           self.input_image_shape, score_threshold=self.score,
    ...
    def detect_image(self, image):
        ...
        box_list=[]#用来存储坐标位置
        score_list=[]#用来存储置信值
        #由于有可能是预测多个类别，因此需要遍历模型，但是我所需要的是仅仅预测一个类别，因此先找出预测的置信值最大的，然后再在循环外面画框
        for i, c in reversed(list(enumerate(out_classes))):
            predicted_class = self.class_names[c]  # 类别,str类型
...
def detect_img_for_test(yolo):
    img_path = '/home/xuy/code/keras-yolo3-shirt_color/people-det-base/JPEGImages/IMG_000067.jpg'
    image = Image.open(img_path)
    r_image = yolo.detect_image(image)
    r_image.show()

    r_image.save('IMG_000067.jpg')
    yolo.close_session()
