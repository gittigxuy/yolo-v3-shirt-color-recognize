...
from shirt_color_recognise.people_classify import shirt_color_classify
    ...
    def detect_image(self,image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            if predicted_class=='person':
                ...
                people_pic=image.crop((left,top,right,bottom))
                # people_pic.show()
                output, colorLabel, color_prob=shirt_color_classify(people_pic)
                # colorText = "color: {} ({:.2f}%)".format(colorLabel, color_prob)
                colorText = "{}".format(colorLabel)
                ...
                if (right-left)<500:
                    ...
                    draw.rectangle(  # 文字背景
                        ...
                        fill=self.colors[c])
                    draw.text(text_origin, colorText, fill=(0, 0, 0), font=font)  # 文案
                    del draw
